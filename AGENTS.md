...
- **Userspace polish** (ring-3 works as of v5.5.0; exec reaps on exit + file
  ...
  - Jobs: `exec <file>` = foreground (blocks via `kwait`), `spawn <file>` =
    background (v5.7.8); `kill` on a running scheduled proc is safe (v5.7.10). Still
    missing: `wait`-any / multiple children (kwait is single-pid).
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  safely. Remaining follow-ups:
  1. **More wait primitives.** `wait`-any / multiple children (kwait is single-pid);
     an IO/event wait queue (keyboard, net) so the compositor can block instead of
     polling `getkey_poll()`; a priority/nice scheme so the compositor beats
     background jobs. Still busy-ish: mtdemo/spin `hlt`-park after finishing.
