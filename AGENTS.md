...
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  background. Remaining follow-ups:
  1. **Retire the legacy blocking path.** `exec` no longer uses
     `switch_to_user_process`/`g_user_proc`/`switch_to_user_trampoline`/the
     setjmp-longjmp unwind — they're unused now. Remove them (mind the web:
     `g_user_proc` is still read by `reap_user_process`, `get_cur_proc`, and the
     scheduler's blocking-exec guard) and drop the dead `SYS_EXIT` longjmp branch.
  2. **Blocked states + wait queues (general).** `kwait` proves the pattern; extend
     ...
     compositor responsive under load.
