...
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  2. **Per-process FS state.** The `ufd` table and `current_dir` (CWD) are global,
     shared by all processes (safe under atomic syscalls, but not isolated). Move
     them into `process_t` for real per-process fds + working directory. Also: the
     FS lock is coarse (whole EXT2 op under `preempt_disable`) — fine for now, but a
     finer-grained buffer-per-op would let FS I/O interleave with other threads.
